...
(async () => {
  // Count patients with regency_code = 'Pohowato'
  const { count, error } = await sb
    .from('patients')
    .select('*', { count: 'exact', head: true })
    .eq('regency_code', 'Pohowato');
  if (error) { console.error('ERR:', error.message); return; }
  console.log('Pasien dengan regency_code="Pohowato":', count);

  // List all distinct regency_code values
  const { data, error: err3 } = await sb
    .from('patients')
    .select('regency_code')
    .not('regency_code', 'is', null);
  if (err3) console.error('ERR3:', err3.message);
  else {
